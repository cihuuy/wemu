...
apt-get update
rm -rf windows2022 windows2022.img windows2022.gz ngrok ngrok.zip ng.sh > /dev/null 2>&1
...
qemu-system-x86_64 -hda windows2022.img -m 8G -smp cores=4 -net user,hostfwd=tcp::3388-:3389 -net nic -object rng-random,id=rng0,filename=/dev/urandom -device virtio-rng-pci,rng=rng0 -vga vmware -nographic &>/dev/null &
clear
echo RDP Address:
